...
export function PaymentTable({ schedule, prepayments }: { schedule: PaymentRow[]; prepayments: Prepayment[] }) {
  ...
  {view === 'month' ? <table><thead><tr><th>№</th><th>Дата</th><th>Платёж</th><th>Проценты</th><th>Тело</th><th>Досрочно</th><th>Остаток</th><th>Событие</th></tr></thead><tbody>{filteredMonths.map((row) => <tr key={row.monthIndex}><td>{row.monthIndex}</td><td>{row.date}</td><td>{formatMoney(row.payment)}</td><td>{formatMoney(row.interest)}</td><td>{formatMoney(row.principal)}</td><td>{formatMoney(row.prepayment)}</td><td>{formatMoney(row.remainingDebt)}</td><td>{row.prepaymentEvents?.length ? row.prepaymentEvents.length === 1 ? `Досрочно ${formatMoney(row.prepaymentEvents[0].amount)} от ${fullDate(row.prepaymentEvents[0].date)}, ${modeLabel(row.prepaymentEvents[0].mode)}` : `Досрочно: ${formatMoney(row.prepayment)}, ${row.prepaymentEvents.length} платежа` : '—'}</td></tr>)}</tbody></table> : <div>Годовой режим сохранён без изменений.</div>}
  </div>;
}
