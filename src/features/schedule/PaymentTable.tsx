...
export function PaymentTable({ schedule, prepayments }: { schedule: PaymentRow[]; prepayments: Prepayment[] }) {
  ...
  const [fromFilter, setFromFilter] = useState<string>(); const [toFilter, setToFilter] = useState<string>();
  const scheduleStart = schedule[0]?.date ?? '';
  const scheduleEnd = schedule.at(-1)?.date ?? '';
  const from = fromFilter && fromFilter >= scheduleStart && fromFilter <= scheduleEnd ? fromFilter : scheduleStart;
  const to = toFilter && toFilter >= scheduleStart && toFilter <= scheduleEnd ? toFilter : scheduleEnd;
  ...
  {view === 'month' && <><div className="filters"><input aria-label="Начало периода" type="date" value={from} onChange={(e) => setFromFilter(e.target.value)} /><input aria-label="Конец периода" type="date" value={to} onChange={(e) => setToFilter(e.target.value)} /><select value={preset} onChange={(e) => setPreset(e.target.value as MonthPreset)}><option value="all">все платежи</option><option value="onlyPrepay">только досрочки</option><option value="onlyInsurance">только страховки</option><option value="prepayYears">только годы с досрочками</option><option value="last12">последние 12 месяцев</option><option value="first12">первые 12 месяцев</option></select><input placeholder="Поиск год/дата" value={search} onChange={(e) => setSearch(e.target.value)} /><button type="button" onClick={() => { setPreset('all'); setSearch(''); setFromFilter(undefined); setToFilter(undefined); }}>Сбросить фильтры</button></div><div className="period-total"><b>Итого по выбранному периоду:</b> платежи и досрочки {formatMoney(periodPayment)} · страховки {formatMoney(periodInsurance)} · реальная сумма {formatMoney(periodPayment + periodInsurance)}</div></>}
