...
export function PaymentTable({ schedule, prepayments }: { schedule: PaymentRow[]; prepayments: Prepayment[] }) {
  ...
  {view === 'month' ? <table><thead><tr><th>№</th><th>Дата</th><th>Платёж</th><th>Проценты</th><th>Тело</th><th>Досрочно</th><th>Страховки / расходы</th><th>Остаток</th><th>Событие</th></tr></thead><tbody>{filteredMonths.map((row) => {
    ...
    return <tr key={row.monthIndex}><td>{row.monthIndex}</td><td>{row.date}</td><td>{formatMoney(row.payment)}</td><td>{formatMoney(row.interest)}</td><td>{formatMoney(row.principal)}</td><td>{formatMoney(row.prepayment)}</td><td>{formatMoney(row.insuranceCost)}</td><td>{formatMoney(row.remainingDebt)}</td><td>{prepayText}{insuranceText !== '—' ? ` · ${insuranceText}` : ''}</td></tr>;
