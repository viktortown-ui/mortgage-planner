...
function App() {
  ...
  useEffect(() => {
    if (normalizedInput.downPayment > normalizedInput.propertyPrice) setError('Первоначальный взнос не может быть больше стоимости недвижимости.');
    else if (!result) setError('Расчёт временно невозможен. Проверьте параметры кредита.');
    else setError('');
