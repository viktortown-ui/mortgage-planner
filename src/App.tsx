...
function ScenarioSection({ results, scenarios, setScenarios }: ScenarioSectionProps) {
  ...
  return (
    <div className="panel section scenario-section">
      ...
      {winner?.result ? <div className="winner-card"><strong>Лучшая стратегия: {winner.id}</strong><span>Она экономит {formatMoney(winner.result.interestSavings)} и снимает {winner.result.monthsSaved} мес. срока относительно базового графика.</span></div> : null}
    </div>
  );
}
